...
using the `kubernetes` profile which is enabled by default. This information is mirrored in the Spring Boot properties files within the
`application.properties` file corresponding which corresponds to the `local` profile, while `application-kubernetes.properties` is used when the
`kubernetes` profile is activated. The `PostgreSQL` connection information is provided in the `application-kubernetes.properties` file to configure the Datasource needed to access the database.
+
...
dekorate.component.name=fruit-backend-sb
