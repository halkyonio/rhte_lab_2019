...
----
echo "LAB_NAME=halkyon_fruit" >> $HOME/.bashrc
source $HOME/.bashrc
...
Create a `halkyon_fruit` development folder on your laptop
----
mkdir $LAB_NAME && cd $LAB_NAME
----
...
POD_ID=$(oc get pod -lapp=$PROJECT -n rhtelab -o name | awk -F '/' '{print $2}')
