...
So, execute the following command providing the following information to each question:
...
Repeat the operation to create a second Spring Boot project using the `crud` template. We'll do it in a non-interactive fashion this time. `hal` is smart enough to check values you provided and only asks you to provide information when needed (e.g. if a provided value is incorrect):
----
...
Create a new git repository, adding the `pom` file you created earlier along with the two Spring Boot applications and push it to your remote repository:
----
git init
...
Package your Spring Boot client application using the following command:
----
...
using the `kubernetes` profile which is enabled by default. This mirrored in the Spring Boot properties files with
`application.properties` corresponding to the `local` profile, while `application-kubernetes.properties` is used when the
`kubernetes` profile is activated. The `PostgreSQL` connection information is provided in the `application-kubernetes.properties` file.
...
This is so that Halkyon knows to activate the Spring `kubernetes` profile, i.e. make Spring Boot uses `application-kubernetes.properties` when
booting instead of the default `application.properties`.

Finally, you can also see that the database connection parameters reference environment variables. These values will be injected
